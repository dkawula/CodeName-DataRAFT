...
function Invoke-DemoVMPrep 
{
  ...
  $vhdxName = 'VMServerBaseCore.vhdx'
  if ($FullServer)
  {
    $FullServer = VMServerBase.vhdx
  }
  
  $null = New-VHD -Path "$($VMPath)\$($GuestOSName).vhdx" -ParentPath "$($BaseVHDPath)\$vhdxName" -Differencing

  Write-Log $VMName 'Creating virtual machine'
  New-VM -Name $VMName -MemoryStartupBytes 4GB -SwitchName $virtualSwitchName -Generation 2 -Path "$($VMPath)\" |
...
$ServerISO = 'c:\ClusterStorage\Volume1\DCBuild\en_windows_server_2016_x64_dvd_9327751.iso'

$WindowsKey = ''; throw 'You did not specify a Windows key. Remove this throw statement when you have.'
...
1..8 | ForEach-Object -Process {
  Invoke-DemoVMPrep "S2DNode$_" "S2DNode$_" -FullServer
}
...
1..8 | ForEach-Object -Process {
  Invoke-NodeStorageBuild "S2DNode$_" "S2DNode$_"
}
